...
Group_stats_results = pd.ExcelWriter(DataSupraDir +'Tumor_repeatability_results.xlsx')
...
for subject in subjs_name:
    ...
    for idx, rater in enumerate(raters_name):
        ...
        with alive_bar(6) as bar:
            ...
            rBV_noLC_vxls = np.vstack(allVoxInt(rBV_noLC, tumorMask))
            rBV_LC_vxls = np.vstack(allVoxInt(rBV_LC, tumorMask))
            rBF_LC_vxls = np.vstack(allVoxInt(rBF_LC, tumorMask))
            MTT_LC_vxls = np.vstack(allVoxInt(MTT_LC, tumorMask))
            
            n_rBV_noLC_vxls = np.vstack(allVoxInt(n_rBV_noLC, tumorMask))
            n_rBV_LC_vxls = np.vstack(allVoxInt(n_rBV_LC, tumorMask))
            n_rBF_LC_vxls = np.vstack(allVoxInt(n_rBF_LC, tumorMask))
            n_MTT_LC_vxls = np.vstack(allVoxInt(n_MTT_LC, tumorMask))
...
plt.savefig(DataSupraDir+'Tumor_ICC.png')     
plt.close()
...
plt.setp(ax.get_yticklabels(), fontsize=16)
fig.tight_layout(pad=2.0)
plt.savefig(DataSupraDir+'Tumor_wCV.png')     
plt.close()
